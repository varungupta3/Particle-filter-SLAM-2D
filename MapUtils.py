...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  for jy in range(0,nys):
    ...
    iy = np.int_(np.round((y1-ymin)/yresolution))
    for jx in range(0,nxs):
      x1 = vp[0,:] + xs[jx] # 1 x 1076
      ix = np.int_(np.round((x1-xmin)/xresolution))
      valid = np.logical_and( np.logical_and((iy >=0), (iy < ny)), \
                                    np.logical_and((ix >=0), (ix < nx)))
      # unique_points = list(set([(ix[valid][i], iy[valid][i]) for i in range(len(valid))]))
      # num_newpts = len(unique_points)
      cpr[jx,jy] = np.sum(im[ix[valid],iy[valid]])/len(valid)
